...
int get_proc_name(int pid, char *name, size_t _size) {
    ...
    if (access(buf, R_OK) == -1 || (fd = open(buf, O_RDONLY | O_CLOEXEC)) == -1)
        return 1;
...
char* getFileData(const char* path) {
    ...
    char *string = static_cast<char *>(malloc(static_cast<size_t>(fsize + 1)));
    fread(string, 1, static_cast<size_t>(fsize), f);
    fclose(f);
